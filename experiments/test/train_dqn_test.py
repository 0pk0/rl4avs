...
from stable_baselines3 import DQN # Import DQN for loading
...
class DQNTrainer(ExperimentRunner):
    ...
    def train_dqn_agent(self, total_timesteps=100000, n_seeds=1, show_training=True, debug_actions=True,
        ...
        if load_model_path:
            print(f"\n🔄 Loading DQN model from {load_model_path} for evaluation...")
            try:
                ...
                model = DQN.load(load_model_path, env=env_for_loading)
                ...
                metrics = self.evaluate_model_wandb(model, eval_env, n_eval_episodes=100, algo_name="DQN", total_timesteps=total_timesteps, run_type="loaded_model_evaluation")
                eval_env.close()
                return [{"seed": 0, "algorithm": "DQN", "evaluation_metrics": metrics, "run_type": "loaded_model_evaluation"}]
        ...
        for seed in range(n_seeds):
            ...
            try:
                ...
                artifact = wandb.Artifact(name=f"DQN_seed_{seed}", type="model")
...
def main():
    ...
    wandb.init(
        ...
        config={
            ...
            "dqn_policy_kwargs": DQN_POLICY_KWARGS,
        }
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train DQN agent with various stopping criteria.")
    ...
    parser.add_argument("--load_model_path", type=str, default=None,
                        help="Path to load a pre-trained DQN model for evaluation.")
